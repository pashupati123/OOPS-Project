...
public class Sphere_Sphere extends JFrame
{
	GamePane gp;			//GamePane is subclass of JComponent on which the puyos are moved
	int width,height;
	int rows,cols;
	int sphere_len;			//holds the length of the each puyo(because it is a square piece)
	Dimension screenSize;	//holds the dimension of the screen interms of resolution
	public Sphere_Sphere()
	{
		super("Sphere-sphere Game By Pashupati And Ravindra");
		...
		sphere_len=(width/8)*2/cols;			//or (height/6)*4/12
		gp=new GamePane(sphere_len,rows,cols);
		...
		setBounds ((width/8)*3-sphere_len*3/2,(height/6)*1-sphere_len,(width/8)*2+sphere_len*3+6,(height/6)*4+25+sphere_len);
		setVisible(true);
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
	}
	public static void main(String args[]) {
		System.out.println("Starting SPHERE-SPHERE Game by PASHUPATI AND RAVINDRA..");
...
class GamePane extends JComponent implements ActionListener
{
	...
	int level,score,pieces,removed_sphere;//pieces is number of joint puyos(single piece) generated
	...
	public GamePane(int l,int r,int c)
	{
		...
		loadImages();
		loadSounds();
		generateSphere();				//to start the generating puyos
		clips[0].loop();
	...
	public void loadImages()//loading images into the image array and pipe objects
	{
		...
		img[i]=tk.getImage("images\\sphere_"+s+(i+1)+".png");
	...
	public void generateSphere()//To generate Puyos at the top of the game window
	{
	...
	public void moveSphere()//Moving generated puyos Downword
	{
		int flag=0;
		...
		erase_sphere();		//so erase puyo which form tetris
	}
	public void erase_sphere()
	{
		int flag=0;
		...
		generateSphere();		//start generating puyos
		if(!timer.isRunning())
		timer.start();
	}
	...
	public void removeAllTetris()
	{
		...
		removed_sphere+=count;
		if(removed_sphere>=50)		//Change the level of the game depending on
	...
	public void paint(Graphics g)
	{
		...
		if(!started)//if game is not started display the information
		{
			...
			g2.setFont(new Font("Ariel",Font.PLAIN,len/3));
			g2.drawString("   Use the left,right and down arrow keys to move the spheres.",0,(rows+1)*len/2);
			g2.drawString("   Pressing the up arrow key rotates the piece.",len,(rows+2)*len/2);
			g2.drawString("  When 4 or more spheres of the same colour are touching",len/3,(rows+3)*len/2);
